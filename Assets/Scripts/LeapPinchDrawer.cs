...
namespace Leap.Unity.DetectionExamples
{
    ...
    public class LeapPinchDrawer : MonoBehaviour
    {
        ...
        void Start()
        {
            _drawStates = new DrawState(this);

           
        }

        int lastLineNumber = 999;
        void Update()
        {
            var drawState = _drawStates;

            if (lastLineNumber != 999)
            {
                if (SocketClient.isDrawLine != 999)
                {
                    if (SocketClient.isDrawLine == lastLineNumber)
                    {
                        drawState.UpdateLine(marker.transform.TransformPoint(SocketClient.drawPointPosition));

                    }


                    if (SocketClient.isDrawLine == 0)
                    {
                        Debug.Log("000000000000000000000000000000000000000000");
                        GameObject[] lines;

                        lines = GameObject.FindGameObjectsWithTag("Line Object");
                        ...
                        foreach (Transform line in lineDrawer.transform)
                        {
            ...
            else
            {
                if (SocketClient.isDrawLine != 999)
                {
                    drawState.BeginNewLine(lineDrawer.transform);
                }
            }

            lastLineNumber = SocketClient.isDrawLine;
        }
        ...
        private class DrawState
        {
            ...
            private LeapPinchDrawer _parent;
            ...
            public DrawState(LeapPinchDrawer parent)
            {
                _parent = parent;
            ...
            public GameObject BeginNewLine(Transform lineDrawer)
            {
                _rings = 0;
                _vertices.Clear();
                _tris.Clear();
                ...
                lineObj.transform.parent = lineDrawer;
                lineObj.tag = "Line Object";
